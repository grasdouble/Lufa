...
async function processTokenFile(category: string, filename: string): Promise<void> {
  ...
  if (exports.length === 0) {
    console.warn(`⚠️  No token exports found in ${sourceFile}`);
    return;
  }
  ...
  console.log(`✅ Generated ${category}/${filename.replace('.ts', '.js')}`);
}
...
function generateIndexFile(): void {
  ...
  // Collect all exports for default export
  const allExports: Array<{ name: string; path: string; isHelper: boolean }> = [];

  // Commented out for now - can be re-enabled later if needed
  indexJS += '// Named exports (commented out - using default export only)\n';
  indexJS += '// Uncomment these if you need tree-shaking via named exports\n';

  // Export all token categories (re-export everything) - COMMENTED OUT
  for (const category of categories.sort()) {
    ...
    for (const file of files) {
      ...
      const modulePath = `./tokens/${category}/${moduleName}.js`;
      indexJS += `// export * from '${modulePath}';\n`;

      // Track exports for default export (read the file to get export names)
      const fileContent = readFileSync(resolve(categoryDir, file), 'utf-8');
      const exportMatches = fileContent.matchAll(/export const (\w+)\s*=/g);
      for (const match of exportMatches) {
        const exportName = match[1];
        // Check if it's a helper function (starts with 'get' or 'is')
        const isHelper = exportName.startsWith('get') || exportName.startsWith('is');
        allExports.push({ name: exportName, path: modulePath, isHelper });
      }
    }
  }

  // Generate named imports for default export
  indexJS += '\n// Import all tokens for default export\n';

  const importsByPath = new Map<string, string[]>();
  for (const exp of allExports) {
    if (!importsByPath.has(exp.path)) {
      importsByPath.set(exp.path, []);
    }
    importsByPath.get(exp.path)!.push(exp.name);
  }

  for (const [path, names] of Array.from(importsByPath.entries()).sort()) {
    indexJS += `import { ${names.join(', ')} } from '${path}';\n`;
  }

  // Generate default export object
  indexJS += '\n// Default export with all tokens\n';
  indexJS += 'const tokens = {\n';

  // Group exports by category for better organization
  const exportsByCategory = new Map<string, typeof allExports>();
  for (const exp of allExports) {
    const category = exp.path.split('/')[2]; // Extract category from path
    if (!exportsByCategory.has(category)) {
      exportsByCategory.set(category, []);
    }
    exportsByCategory.get(category)!.push(exp);
  }

  // Generate default export content, organized by category
  for (const [category, exports] of Array.from(exportsByCategory.entries()).sort()) {
    indexJS += `  // ${category.toUpperCase()}\n`;
  ...
  indexJS += '};\n\n';
  indexJS += 'export default tokens;\n';

  writeFileSync(resolve(distDir, 'index.js'), indexJS);
  console.log('✅ Generated index.js with default export');
}
...
async function generateJS() {
  console.log('🔨 Generating JavaScript files with CSS variable references...\n');
  ...
  try {
    ...
    console.log('\n✅ JavaScript generation complete!');
  } catch (error) {
