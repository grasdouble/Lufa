    ...
    render: () => (
        <Stack direction="vertical" gap="spacious">
            {(Object.values(GRID_COLUMNS) as Array<keyof typeof GRID_COLUMNS>).map((columns) => (
                <div key={columns}>
                    <div style={{ fontFamily: 'monospace', color: color.text.secondary, marginBottom: '12px' }}>columns: {columns}</div>
                    <Container
                        size="xl"
                        paddingX="none"
                        style={{ backgroundColor: color.background.primary, border: `1px solid ${color.border.light}`, borderRadius: '12px', padding: '16px' }}
                    >
    ...
    render: () => (
        <Stack direction="vertical" gap="spacious">
            {(Object.values(GRID_GUTTER) as Array<keyof typeof GRID_GUTTER>).map((gutter) => (
                <div key={gutter}>
                    <div style={{ fontFamily: 'monospace', color: color.text.secondary, marginBottom: '12px' }}>gutter: {gutter}</div>
                    <Container
                        size="xl"
                        paddingX="none"
                        style={{ backgroundColor: color.background.primary, border: `1px solid ${color.border.light}`, borderRadius: '12px', padding: '16px' }}
