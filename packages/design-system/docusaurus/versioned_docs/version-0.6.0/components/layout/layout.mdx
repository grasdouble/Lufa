...
import {
  ...
  SidebarPosition,
  SidebarWidth,
} from '@site/src/dsExamples/layout/layout';
...
```tsx
<Layout sidebar>
  <Layout.Header aria-label="Site header">{/* header content */}</Layout.Header>
  <Layout.Sidebar aria-label="Table of contents">{/* TOC content */}</Layout.Sidebar>
  <Layout.Content aria-label="Article content">{/* main content */}</Layout.Content>
  <Layout.Footer aria-label="Site footer">{/* footer content */}</Layout.Footer>
</Layout>
```
...
```tsx
<Layout sidebar>
  <Layout.Header aria-label="Global header">{/* header */}</Layout.Header>
  ...
  <Layout.Footer aria-label="Site footer">{/* footer */}</Layout.Footer>
</Layout>
```
...
function AppLayout({ children }: { children: ReactNode }) {
  return (
    <Layout sidebar sidebarWidth={LAYOUT_SIDEBAR_WIDTH.default} gap={LAYOUT_GAP.md}>
      ...
      <Layout.Footer>
        <div className="px-6 py-4 text-sm text-gray-600">© 2024 Your Company. All rights reserved.</div>
      </Layout.Footer>
    </Layout>
  );
}
...
function DashboardLayout() {
  return (
    <Layout sidebar sidebarWidth={LAYOUT_SIDEBAR_WIDTH.wide} gap={LAYOUT_GAP.lg}>
      ...
      <Layout.Footer>
        <div className="px-6 py-3 text-xs">Last updated: {new Date().toLocaleString()}</div>
      </Layout.Footer>
    </Layout>
  );
}
...
function CollapsibleLayout({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <Layout.Footer>
        <div className="px-4 py-3">{!collapsed && <FooterContent />}</div>
      </Layout.Footer>
    </Layout>
  );
}
