...
import {
  ...
  StatusPosition,
  Variant,
} from '@site/src/dsExamples/display/avatar';
...
function UserProfile({ user }) {
  return (
    <div className="profile-header">
      <Avatar src={user.avatarUrl} alt={user.name} size="xl" status={user.onlineStatus} />
      <div>
        <h1>{user.name}</h1>
        <p>{user.title}</p>
      </div>
...
function TeamList({ members }) {
  return (
    <ul className="team-list">
      {members.map((member) => (
        <li key={member.id}>
          <Avatar src={member.avatar} alt={member.name} size="md" status={member.status} />
          <div>
...
function UserStatus({ user }) {
  ...
  return (
    <div className="user-status">
      <Avatar src={user.avatar} alt={user.name} size="lg" status={user.status} statusPosition="bottom" />
      <div>
        <h3>{user.name}</h3>
