...
import { LiveDemo } from '../../src/dsExamples/components/badge';
...
- Status indicators (active, pending, error, warning)
...
Badge provides five semantic variants, three sizes, and an optional dot indicator.

```tsx
import { Badge } from '@grasdouble/lufa_design-system';

export function BadgeVariantExamples() {
  return (
...
export function AccessibleBadges() {
  return (
    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
      ...
      <Badge as="label" htmlFor="email" variant="info">Email Address</Badge>
    </div>
  );
}
```
...
- [Text](/docs/primitives/text) - For displaying text content alongside badges
- [Button](/docs/components/button) - For actionable elements
- [Icon](/docs/primitives/icon) - For icon indicators
