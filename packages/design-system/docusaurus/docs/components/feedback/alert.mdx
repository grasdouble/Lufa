...
import {
  ...
  LiveDemo,
  Variant,
} from '@site/src/dsExamples/feedback/alert';
...
```tsx
<Alert variant="error" title="Form Error" aria-describedby="error-details">
...
function ProfileForm() {
  ...
  return (
    <>
      {showSuccess && (
        <Alert variant="success" title="Saved" closable onClose={() => setShowSuccess(false)}>
...
function TipAlert() {
  ...
  return (
    <Alert variant="info" title="Pro Tip" closable onClose={handleClose}>
...
function ErrorAlert({ error, onRetry, onDismiss }) {
  return (
    <Alert variant="error" title="Failed to Load Data" closable onClose={onDismiss}>
      <div>
        <p>{error.message}</p>
        <Button size="small" variant="outlined" onClick={onRetry} style={{ marginTop: '12px' }}>
          Retry
        </Button>
      </div>
    </Alert>
