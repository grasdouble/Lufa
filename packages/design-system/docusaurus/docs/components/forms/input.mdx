...
import {
  ...
  TypesExample,
  VariantExample,
} from '@site/src/dsExamples/forms/input';
...
```tsx
<Input label="Username" helperText="Must be 3-20 characters" aria-describedby="username-helper" />
...
```tsx
<Input label="Email" error="Please enter a valid email address" type="email" />
...
```tsx
<Input label="Username" aria-describedby="username-helper username-error" aria-label="User account name" />
```
...
function SearchBar() {
  return <Input placeholder="Search products..." type="search" startAdornment={<SearchIcon />} fullWidth />;
}
```
