...
description: Visual separator for grouping content.
sidebar_position: 7
---
...
import {
  ...
  LineStyles,
  LiveDemo,
} from '../../src/dsExamples/primitives/divider';
...
- `horizontal` (default) uses `<hr>` for semantic separation.
- `vertical` uses `<div>` and is best inside flex or grid containers.

### Variant

- `subtle`, `default`, `strong` control visual weight via tokens.
...
- `compact` (8px), `default` (16px), `comfortable` (24px).

### Line style

- `solid` (default) and `dashed`.
